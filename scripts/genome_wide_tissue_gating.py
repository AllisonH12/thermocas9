...
def compute_all(scored_path: Path, positives: set[str]) -> dict:
    ...
    for cid, chrom, mu_t, q25_t, q75_t, mu_n, q25_n, q75_n, p_t, p_r, v25_s in stream_records(scored_path):
        ...
        for axis_name, score in (
            ...
            *(
                (f"gap_sigmoid_{b:.4f}",
                 p_t * gap_sigmoid_score(mu_t, mu_n, b) * p_r)
                for b in BANDWIDTHS
            ),
        ):
...
def main() -> None:
    ...
    r = compute_all(args.scored, positives)
    ...
    md = [f"# Genome-wide tissue gating: shipped V2.5 vs gap_sigmoid on {args.scored.name}",
          "",
          f"Generated by `scripts/genome_wide_tissue_gating.py`. δ = {DEFAULT_DELTA}.",
