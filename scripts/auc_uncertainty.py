...
def main() -> None:
    ...
    ap.add_argument("--n-perm", type=int, default=DEFAULT_N_PERM,
                    help=f"permutation-null draws per cohort × axis (default {DEFAULT_N_PERM:,}). "
                          "Use 1_000_000 to resolve p below the 10⁴-draw floor.")
    ap.add_argument("--n-boot", type=int, default=DEFAULT_N_BOOT,
                    help=f"negative-bootstrap resamples (default {DEFAULT_N_BOOT:,})")
    ap.add_argument("--output-prefix", type=Path, default=None,
                    help=f"override default output path (default: examples/auc_uncertainty.{{tsv,md}})")
    args = ap.parse_args()

    n_perm = args.n_perm
    n_boot = args.n_boot
    out_tsv = (args.output_prefix.with_suffix(".tsv") if args.output_prefix else OUT_TSV)
    out_md = (args.output_prefix.with_suffix(".md") if args.output_prefix else OUT_MD)

    positives = set(line.strip() for line in POSITIVES.read_text().splitlines() if line.strip())
    print(f"positives: {sorted(positives)}")
    print(f"running permutation null at N = {n_perm:,} draws; bootstrap at N = {n_boot:,}")
    ...
    md = ["# AUC uncertainty — n_pos = 3 primary endpoint",
          ...
          f"The p-value resolution floor is 1/(n_perm+1) ≈ {1.0/(n_perm+1):.2g}.",
