...
Code at <https://github.com/AllisonH12/thermocas9>, immutable tag `memo-2026-04-22-as`.
...
**Figure 2.** Cross-cohort AUC by score axis — 4 axes (Δβ-only, V1 `final_score`, V2 `tumor_only`, V2.5-diff `tumor_plus_differential_protection`) × 4 cohort paths (GSE322563 HM450, GSE322563 native EPIC v2, GSE77348, GSE69914) × 3 label tiers. **Historical V2.5-diff sensitivity figure** retained for reference and AUC parity with pre-`memo-2026-04-22-as` scored JSONLs; the current recommended prioritization axis is V2.5-sigmoid (`tumor_plus_gap_sigmoid`), and the limma-style moderated-t DMR baseline is now reported alongside the four axes in §5.1 / §5.3 (rows added in revision; PAPER.md §5.2.2 has the full whole-genome panel including V2.5-sigmoid and limma-style moderated-t). **(a)** Validated-label AUC (`n_pos = 3`, Roth Fig. 5d target probes). V2.5-diff is the highest axis on all four cohort paths at the validated tier except GSE69914 tissue, where V2 `tumor_only` (0.80) leads V2.5-diff (0.77) on raw AUC; V2 `tumor_only`'s `tie_band@100 = 6,540` (§5.3) disqualifies it from discovery use, making V2.5-diff the highest *usable* discovery axis among the four shown there — but on the same tissue cohort V2.5-sigmoid reaches 0.862 (PAPER.md §5.2.2 / §5.3 of this manuscript) and is the recommended tissue axis. **(b)** Sensitivity across label granularities (validated / narrow ±50 bp / wide ±500 bp). Δβ-only (green) collapses on GSE69914 tissue at narrow and wide while V1 also degrades sharply; V2.5-diff and V2 `tumor_only` maintain discriminative power. Dashed line at AUC = 0.5 (random). The GSE68379 out-of-distribution cohort is not included here: Δβ-only, V2 `tumor_only`, and V2.5-diff invert below 0.5 on all three label tiers (AUCs 0.15–0.20 for Δβ, 0.22–0.46 for V2, 0.17–0.27 for V2.5-diff); V1 is near-random (0.41–0.51) rather than cleanly inverted (§5.4 reports separately).
...
1. **V2.5-diff matches the highest-AUC discovery axis at every matched-cell-line and tissue cohort × tier combination tested. V2.5-sigmoid matches V2.5-diff on AUC *on matched cell lines* (within 0.002 across GSE322563 HM450, GSE322563 native EPIC v2, and GSE77348; PAPER.md §5.2.2 WG panel) and strictly beats V2.5-diff on WG top-K usability — `tie_band@100 = 1` vs 421 / 1,127 / 1,493 under the WG denominator. *On tissue (GSE69914), V2.5-sigmoid beats V2.5-diff by +0.05 to +0.08 AUC* (0.862 vs 0.778 at the validated tier), so the parity statement is cell-line-only and the tissue win is separate.** V2.5-sigmoid ships as `tumor_plus_gap_sigmoid` in this tag and is the recommended probabilistic prioritization axis per PAPER.md §5.2.2 whole-genome panel. (The OOD boundary case GSE68379 correctly inverts for all axes and is reported separately in §5.4; it is not pooled with the discovery-mode comparisons here.) Row-by-row margins against the shipped baselines:
...
2. **The recommended probabilistic axis is `gap_sigmoid`** (`probabilistic_mode: tumor_plus_gap_sigmoid`, ships in this tag). PAPER.md §5.2.2 cross-cohort WG panel shows `gap_sigmoid` is uniformly equal-or-better to shipped V2.5 on every tested non-boundary cohort × dimension combination: AUC within 0.002 on matched cell-line (GSE322563 HM450 0.989 vs 0.989, native v2 0.998 vs 0.998, GSE77348 0.982 vs 0.982), AUC +0.08 on tissue (GSE69914 0.862 vs 0.778), and critically `tie_band@100 = 1` at WG scale on every matched cell-line cohort (vs V2.5's 421–1,493 on the same WG catalogs, where V2.5's top-K becomes unusable). Shipped V2.5 (`probabilistic_mode: tumor_plus_differential_protection`) is retained as a selectable mode for backward compatibility and AUC parity on cell-line cohorts but is no longer recommended over `gap_sigmoid` on any tested regime. For GSE68379 (OOD cross-series), no axis is supported.
...
3. **Random-chromosome-matched negative-universe controls** (the one remaining untested denominator dimension — the WG half of the negative-universe sensitivity is done in §5.2.2).
4. **Optional R `limma` parity check.** Our limma-style moderated-t baseline is a pure-python Smyth (2004) implementation (§4.3, §5.2.2); a supplementary R-vs-python parity table on a small matrix would close the remaining methods-journal concern about faithfulness to the canonical `limma::lmFit` + `limma::eBayes` call path.
...
From a fresh clone of the repository at `memo-2026-04-22-as`:
