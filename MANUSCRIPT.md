...
Code at <https://github.com/AllisonH12/thermocas9>, immutable tag `memo-2026-04-22-af`.
...
The tissue-cohort picture is regime-specific. Shipped V2.5 is the recommended probabilistic axis on matched cell-line cohorts (§5.2.1 confirms its cell-line advantage is structural, not p_diff-shape-specific: sigmoid and hard-threshold ablations match it within 0.001–0.002 AUC). On primary tissue, the `gap_sigmoid` formulation outperforms shipped V2.5 by +0.05 to +0.09 AUC on GSE69914, at both chr5/6/10 and whole-genome (~20M candidate) probe-window denominators (PAPER.md §5.2.2); it is documented and will ship as an explicit `probabilistic_mode` option in the next release. V1 remains the stable-release default because of its continuous-score guarantees (deterministic `tie_band = 1`), not because it wins AUC anywhere — §6.2 explains the shipping logic.
...
2. **The recommended probabilistic axis is regime-specific.** Shipped V2.5 (`probabilistic_mode: tumor_plus_differential_protection`) on matched cell-line cohorts — highest-AUC shipped axis there, with §5.2.1 showing the advantage is structural rather than `p_diff`-shape-specific. `gap_sigmoid` on primary tissue — PAPER.md §5.2.2 shows it beats shipped V2.5 by +0.05 to +0.09 AUC on GSE69914 at both chr5/6/10 and whole-genome denominators; documented, shipping as an explicit mode option in the next release. For GSE68379 (OOD cross-series), no axis is supported.
...
From a fresh clone of the repository at `memo-2026-04-22-af`:
