...
Code at <https://github.com/AllisonH12/thermocas9>, immutable tag `memo-2026-04-22-bx`. The long-form companion (`PAPER.md` at tag `paper-5-10j`) carries the per-section `§X.Y` cross-references this trim cites; both tags point into the same repository.

---

## Abstract

**Motivation.** Methylation of the fifth PAM cytosine protects loci from ThermoCas9 cleavage (Roth et al., *Nature* 2026), enabling tumor-selective targeting where methylation diverges between cancer and matched normal tissue. Turning this into a target-prioritization shortlist requires a scorer that is cohort-agnostic, honest about evidence quality, and explicit about ties — none was publicly available, and the obvious baseline of ranking by Δβ = β_normal − β_tumor had not been benchmarked against any alternative.

**Results.** We present `thermocas`, an open probabilistic scoring framework built around a *compositional skeleton* `p_targ × (gap factor) × p_trust` paired with a tie-band-aware benchmarking contract reporting `precision_at_k_{min, max}`, `tie_band_size_at_k`, and mid-rank Mann–Whitney AUC on every result. Two gap-factor instances ship: V2.5-diff (`p_diff = P(β_normal − β_tumor > δ)`) and V2.5-sigmoid (a fixed-bandwidth logistic on the same gap). We benchmark across four public cohorts (GSE322563, GSE77348, GSE69914 tissue, GSE68379) against five baselines including raw Δβ and a limma-style moderated-`t` DMR baseline. On matched cell lines Δβ is already strong (AUC 0.96–0.97); V2.5 variants add small descriptive margins, and a 1,000,000-draw random-triple null places V2.5-diff at one-sided `p ≈ 4 × 10⁻⁶`–`2.8 × 10⁻⁵`. The headline result is on bulk tissue: limma-style collapses to AUC 0.573 while V2.5-sigmoid recovers to 0.862 — the `p_targ × p_trust` *wrapping*, not the gap factor, is the load-bearing contribution. A frozen whole-genome panel shows V2.5-sigmoid matches V2.5-diff's cell-line AUC within 0.001 but collapses WG `tie_band@100` from 421–1,493 to 1. Honest scope: validated set is n = 3, AUC is a rank-lift summary, tissue gain is per-positive heterogeneous (ESR1 matched-near-random under V2.5-sigmoid), and the framework is not prospectively validated. V2.5-sigmoid is recommended for new non-boundary prioritization; V1 remains the package default for its `tie_band = 1` guarantee.

**Availability and implementation.** Open source at <https://github.com/AllisonH12/thermocas9>, tagged `memo-2026-04-22-bx` for the version evaluated here. 249 unit tests pass under `uv run pytest -q`. Python 3.11+, BSD-3 licensed.
...
3. A benchmark against five baselines — a Δβ-only ranker, an uncertainty-aware Δβ_z, V1 deterministic scoring, the V2 `tumor_only` diagnostic, and a sample-level limma-style moderated-`t` DMR baseline (Smyth 2004 empirical-Bayes; canonical R `limma::lmFit + eBayes` parity reported in PAPER.md §5.8) — across four public cohorts with positives tiered from Roth's own validated targets to a wide genomic window. The two V2.5 generation gap factors (V2.5-diff and V2.5-sigmoid) are also benchmarked head-to-head as the final-method comparison.
...
on cell-line cohorts; V2.5-sigmoid is the worked/default whole-genome
prioritization axis selected in PAPER.md §5.2.2, with
restricted-universe caveats in PAPER.md §5.7 and §6.1.
...
| **V2** (deprecated; reported for completeness) | `p_targ × p_prot × p_trust` with `p_prot = P(β_normal > 0.5)` | First-pass composite with a static threshold on the normal side. Empirically anti-predictive (AUC 0.384 on the GSE77348 cell-line surrogate per PAPER.md §5.2.1 / Appendix A factor-ablation table); rejected. Not carried forward as a recommended axis. |
...
| cohort | tier (n_pos) | Δβ-only | V1 | V2 tumor_only | V2.5-diff |
|---|---|---:|---:|---:|---:|
...
- **V2.5-sigmoid is the recommended tissue prioritization axis.** V2.5-diff reaches +0.113 / +0.172 / +0.291 AUC over V1 on validated / narrow / wide with stable top-K (`tie_band = 2`); V2.5-sigmoid additionally beats V2.5-diff by +0.05 to +0.09 AUC across the PAPER.md §5.2.1 bandwidth family and at the WG denominator in PAPER.md §5.2.2 (0.862 vs 0.773 at σ_fixed ≈ 0.0707 on the validated label set). Both ship as selectable modes in this tag. The shipping hierarchy is three-layered and worth keeping distinct: **V1 `final_score`** is the stable package default (continuous score, `tie_band = 1` by construction); **`tumor_only`** is the default `probabilistic_mode` enum value for backward compatibility (users opt into V2.5-diff or V2.5-sigmoid via cohort-YAML); and **V2.5-sigmoid** is the recommended probabilistic prioritization axis once a probabilistic mode is selected. V2.5-diff is retained for AUC-parity use against pre-V2.5-sigmoid scored JSONLs. The tissue-specific gain of V2.5-sigmoid over V2.5-diff is a direct consequence of the PAPER.md §3.5 binding-rate picture: on cell lines `σ_floor` binds almost everywhere and V2.5-diff's `p_diff` collapses to a fixed-bandwidth response anyway (so the two modes are equivalent there); on tissue the per-site σ varies and hurts the ranking, so the explicit fixed-bandwidth V2.5-sigmoid wins.

**The cross-cohort picture after whole-genome validation supports `gap_sigmoid` for new prioritization runs, with three precise claims.** PAPER.md §5.2.2 cross-cohort panel: (1) AUC-equivalent to shipped V2.5 on all three matched cell-line cohorts (within 0.001); (2) `tie_band@100 = 1` for `gap_sigmoid` vs 421–1,493 for shipped V2.5 across the three WG matched cell-line cohorts — V2.5's top-K is unusable at WG scale on n = 2/2 or 3/3 cohorts, while `gap_sigmoid`'s is clean; (3) higher transported-label all-positive AUC on the single tissue cohort tested (+0.05 to +0.08; per-positive heterogeneous — GATA3 carries the matched-pool signal while ESR1 is matched-near-random under V2.5-sigmoid in PAPER.md §5.9, and the EvidenceClass-restricted `EXACT + PROXIMAL_CLOSE` ESR1 universe shows V2.5-sigmoid trailing V2.5-diff / Δβ-only / limma-style in PAPER.md §5.7). `gap_sigmoid` ships as `probabilistic_mode: tumor_plus_gap_sigmoid` in this tag. Shipped V2.5 (`tumor_plus_differential_protection`) is retained as a selectable mode for backward compatibility but is no longer the recommended prioritization axis for new non-boundary runs. V1 remains the stable-release default because of its continuous-score guarantees (deterministic `tie_band = 1`), not because it wins AUC anywhere — §6.2 explains the shipping logic.
...
**Figure 3.** Tie-window-aware top-20 gene presence. Columns are per-cohort per-axis top-20 shortlists (ordered by the deterministic tie-break documented in §3.1); rows are the union of nearest-gene symbols sorted by cross-column presence. Column-header colors: **blue** = deterministic top-K under the current score distribution (`tie_band = 1` for V1, `tie_band = 2` for V2.5 on high-`n` tissue); **red** = top-K is a 20-record window inside a large tied band (`tie_band = 190`, `299`, `421` for V2.5 on low-`n` cell-line cohorts), so membership is the documented tie-break policy's choice, not the scorer's discrimination. Genes bolded in blue on the row labels are present in **all three** cell-line V2.5 shortlists (GSE322563 HM450, GSE322563 native, GSE77348) — cross-platform / cross-laboratory convergence gives two such genes: *CELF2* and *XPNPEP1* (the same two are also two of the four genes that PAPER.md §5.6 reports as the GSE322563↔GSE77348 V2.5-diff top-20 2-way intersection: *ADRB1*, *CELF2*, *GFRA1*, *XPNPEP1*). **The tissue cohort (GSE69914 V2.5) has zero nearest-gene overlap with any of the three cell-line V2.5 shortlists** — its top-20 candidates map to exactly 9 distinct nearest-gene symbols (*COL21A1, FOXC1, FOXCUT, MAS1L, MAT2B, MXI1, RUNX2, SCML4, TENM2*), all of which are disjoint from every cell-line V2.5 top-20. This is a direct visualization of the cell-line-vs-tissue biological shift that drives the AUC patterns in Figure 2.

### 5.6 Cross-cohort, EvidenceClass-controlled, and feature-matched-control tables

The §6 recommendation depends on the whole-genome (WG) panel, the
EvidenceClass-restricted reversal, and the feature-matched
negative-universe controls. The three decisive tables — committed
under `examples/genome_wide_panel.tsv`,
`examples/evidence_class_stratified_benchmark.tsv`, and
`examples/feature_matched_negative_controls.tsv` respectively — are
reproduced inline here so this manuscript stands alone for a reviewer
who does not consult PAPER.md §5.2.2 / §5.7 / §5.9.

**(a) Whole-genome panel: V2.5-sigmoid vs V2.5-diff vs limma-style.**
WG validated-label AUC (n_pos = 3) and `tie_band@100` across the four
evaluated cohort paths:

| cohort | V2.5-diff AUC | V2.5-sigmoid AUC | limma-style AUC | V2.5-diff tie@100 | V2.5-sigmoid tie@100 | limma-style tie@100 |
|---|---:|---:|---:|---:|---:|---:|
| GSE322563 HM450     | 0.989 | 0.988 | 0.959 | 1,127 | 1 |  91 |
| GSE322563 native v2 | 0.998 | 0.998 | 0.991 |   421 | 1 |  39 |
| GSE77348            | 0.982 | 0.981 | 0.962 | 1,493 | 1 | 115 |
| GSE69914 (tissue)   | 0.778 | 0.862 | 0.573 |     1 | 6 |  57 |

V2.5-sigmoid is AUC-equivalent to V2.5-diff on the three matched
cell-line cohorts (within 0.001), eliminates V2.5-diff's 421–1,493-record
WG tied bands, and is +0.05 to +0.08 transported-label AUC on the
single tissue cohort tested. limma-style moderated-`t` is competitive
on cell lines but collapses to AUC 0.573 on tissue.

**(b) EvidenceClass-restricted ESR1 reversal on GSE69914 tissue.** On
the high-confidence `EXACT + PROXIMAL_CLOSE` GSE69914 universe, only
ESR1 is evaluable (none of the three Roth-validated positives is
EXACT-class; ESR1 is PROXIMAL_CLOSE, GATA3 is PROXIMAL, EGFLAM is
REGIONAL). V2.5-sigmoid trails the other axes on this single-positive
restricted-universe AUC:

| axis | `EXACT + PROXIMAL_CLOSE` GSE69914 ESR1 AUC |
|---|---:|
| V2.5-sigmoid | **0.610** |
| V2.5-diff    | 0.922 |
| Δβ-only      | 0.958 |
...
This is the only axis × universe cell on tissue where V2.5-sigmoid is
not the top axis. The mechanism is the smooth-sigmoid response: in
the high-confidence subset, V2.5-sigmoid's saturating gap factor
compresses ESR1's tissue Δβ signal more than V2.5-diff's discrete-step
`p_diff` does. We report this because it materially limits the tissue
stress-test interpretation, but it is one cell out of the broader
all-positive WG result above and does not revise the §6.2
recommendation.

**(c) Feature-matched negative-universe empirical *p*-values.** Each
Roth positive ranked against same-`(EvidenceClass, pam_family,
is_cpg_pam, chrom)`-signature negatives; empirical *p* is the fraction
of matched negatives that score strictly better than the positive
(ties contribute 0.5; one-sided null). Bold marks V2.5-sigmoid cells
with *p* < 0.05 because it is the recommended axis (does **not** mark
the row-best axis):
...
On every matched-cell-line cell (9/12 rows), every positive sits in
the top ~4% of its feature-matched pool under V2.5-sigmoid (`p` ∈
[0.0018, 0.0390]). On GSE69914 tissue, the matched-pool signal
collapses onto GATA3 alone (`p` = 0.019, 98th matched-percentile),
with ESR1 matched-near-random under V2.5-sigmoid (`p` = 0.44) and
...
**Pre-registered Roth System B HEK293T / HCT116 extension.** We
pre-registered an independent Roth System B extension on the
...
target-side editability check are in PAPER.md Appendix B. Reproduce
with `uv run python scripts/score_roth_transport_subset.py`.
...
On matched cell-line cohorts — where the PAM cytosine's methylation state cleanly separates the tumor and normal arms — raw Δβ is already a strong baseline and V2.5 improves on it by a consistent but small margin (+0.010 to +0.080 across all nine tier × path rows on GSE322563 and GSE77348). **V2.5 does not dominate Δβ by a large AUC margin on easy cohorts, and we do not claim it does** — the practical case for V2.5 over Δβ on such cohorts is structural, not a matter of AUC supremacy. What V2.5 *does* do, relative to the published baselines (Δβ-only, Δβ_z, V1, V2 `tumor_only`, limma-style moderated-`t`), is summarized below:

1. **V2.5-sigmoid is the recommended probabilistic prioritization axis, supported by three precise claims rather than a blanket dominance:** (a) AUC-equivalent to V2.5-diff on all three matched cell-line cohorts (within 0.001 across GSE322563 HM450, GSE322563 native EPIC v2, and GSE77348; PAPER.md §5.2.2 WG panel); (b) strictly better WG top-K usability — `tie_band@100 = 1` for V2.5-sigmoid vs 421 / 1,127 / 1,493 for V2.5-diff under the WG denominator on the three matched cell-line cohorts; (c) higher transported-label all-positive AUC on the single tissue cohort tested — V2.5-sigmoid beats V2.5-diff by +0.05 to +0.08 on GSE69914 (0.862 vs 0.778 at the validated tier). The tissue gain is per-positive heterogeneous (PAPER.md §5.9: GATA3 carries the matched-pool signal, ESR1 is matched-near-random under V2.5-sigmoid; PAPER.md §5.7: in the EvidenceClass-restricted `EXACT + PROXIMAL_CLOSE` ESR1 universe, V2.5-sigmoid trails V2.5-diff / Δβ-only / limma-style). V2.5-sigmoid ships as `tumor_plus_gap_sigmoid` in this tag; V2.5-diff is retained for backward compatibility and AUC parity. (The OOD boundary case GSE68379 correctly inverts for all axes and is reported separately in §5.4; it is not pooled with the prioritization-mode comparisons here.) Row-by-row margins against the shipped baselines:
...
This is the methodologically honest framing: Δβ is a strong baseline that should be reported alongside any new methylation-guided Cas9 scorer; `gap_sigmoid` (shipped in this tag as `tumor_plus_gap_sigmoid`) is the recommended probabilistic prioritization axis across every non-boundary cohort shape we tested, supported by three precise claims rather than a blanket dominance: (1) AUC-equivalent to shipped V2.5 on all three matched cell-line cohorts (within 0.001, PAPER.md §5.2.2); (2) `tie_band@100 = 1` for `gap_sigmoid` vs 421–1,493 for shipped V2.5 across the three WG matched cell-line cohorts — V2.5's top-K is unusable at WG scale on n = 2/2 or 3/3 cohorts while `gap_sigmoid`'s is clean; (3) higher transported-label all-positive AUC on the single tissue cohort tested, with explicit per-positive heterogeneity (GATA3 carries the matched-pool signal; ESR1 is matched-near-random under V2.5-sigmoid in PAPER.md §5.9, and within the EvidenceClass-restricted `EXACT + PROXIMAL_CLOSE` ESR1 universe V2.5-sigmoid trails V2.5-diff / Δβ-only / limma-style per PAPER.md §5.7). Shipped V2.5 is retained as a selectable mode for backward compatibility and AUC parity. V1 remains the stable-release default for reasons of backward compatibility and its deterministic `tie_band = 1` top-K — *not* because V1 wins AUC anywhere. §6.2 below states the shipping logic explicitly.
...
|---|---|
| New probabilistic prioritization runs | **V2.5-sigmoid** (`tumor_plus_gap_sigmoid`). Hypothesis generation only; AUC-equivalent to V2.5-diff on matched cell lines (within 0.001), avoids V2.5-diff's low-`n` WG tied-band blowup (`tie_band@100 = 1` vs 421–1,493), higher transported-label AUC on the single tissue cohort tested (per-positive heterogeneous; see PAPER.md §5.4 / §5.7 / §5.9). |
...
Two bodies of prior art are adjacent but non-overlapping. Generic CRISPR guide-scoring tools (e.g. Azimuth, DeepCRISPR, CRISPRitz) score sequence properties and off-target risk; none of them incorporate methylation state at the PAM cytosine, because none of them target a methylation-sensitive Cas9 variant. Methylation-aware variant analysis tools (e.g. methylKit, minfi) provide per-probe differential analysis on arrays but do not produce ranked target lists; their output feeds a scorer, not replaces one. `thermocas` is the first open framework we are aware of that joins the two — a ranking scorer that consumes methylation array summaries and outputs tumor-selective ThermoCas9 target shortlists with honest uncertainty. Probe-level differential-methylation callers themselves are not excluded as comparators: a limma-style moderated-`t` baseline (Smyth 2004 empirical-Bayes variance prior, pure-Python implementation with R `limma::lmFit + eBayes` parity) is reported alongside V2.5 across every cohort × tier in §5.6, where it is competitive on matched cell lines but collapses to AUC 0.573 on the GSE69914 tissue cohort under transported Roth labels.
...
4. **Canonical R `limma` parity** — completed at `memo-2026-04-22-ba`. Per-probe Spearman/Pearson `t_mod` ≥ 0.9997 across the three R-parity cohorts (GSE69914 tissue / GSE77348 / GSE322563 HM450) versus R `limma::lmFit + eBayes` 3.66.0 on identical sample-level β + group inputs; full results in PAPER.md §5.8 with per-cohort artifacts under `examples/r_limma_parity_*.{tsv,md}`.
...
From a fresh clone of the repository at `memo-2026-04-22-bx` with the
above raw inputs staged:

```bash
# One-time env setup
uv sync
uv run pytest -q           # 249 passing
