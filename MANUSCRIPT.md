...
Code at <https://github.com/AllisonH12/thermocas9>, immutable tag `memo-2026-04-22-ac`.
...
**Availability and implementation.** `thermocas` is open source at <https://github.com/AllisonH12/thermocas9>, tagged `memo-2026-04-22-ac` for the version evaluated here. 236 unit tests pass under `uv run pytest -q`. Python 3.11+, BSD-3.
...
The tissue-cohort picture tightens the decision hierarchy: V2.5 is the recommended research mode on both matched cell-line *and* tissue cohorts **among the shipped benchmark modes**, not just matched cell-line. On tissue it is the best shipped usable axis, not the best tested probabilistic formulation. V1 remains the stable-release default because of its continuous-score guarantees, not because it wins AUC anywhere — §6.2 explains the shipping logic.
...
From a fresh clone of the repository at `memo-2026-04-22-ac`:
