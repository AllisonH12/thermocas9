...
Code at <https://github.com/AllisonH12/thermocas9>, immutable tag `memo-2026-04-22-bb`.
...
**Availability and implementation.** `thermocas` is open source at <https://github.com/AllisonH12/thermocas9>, tagged `memo-2026-04-22-bb` for the version evaluated here. 245 unit tests pass under `uv run pytest -q`. Python 3.11+, BSD-3.
...
4. **Canonical R `limma` parity** — completed at `memo-2026-04-22-bb`. Per-probe Spearman/Pearson `t_mod` ≥ 0.9997 across all three primary cohorts versus R `limma::lmFit + eBayes` 3.66.0 on identical sample-level β + group inputs; full results in PAPER.md §5.8 with per-cohort artifacts under `examples/r_limma_parity_*.{tsv,md}`.
...
From a fresh clone of the repository at `memo-2026-04-22-bb`:
