...
Code at <https://github.com/AllisonH12/thermocas9>, immutable tag `memo-2026-04-22-ao`.
...
3. **σ_floor = 0.05** is empirical. A sweep over σ_floor ∈ {0.02, 0.05, 0.10, 0.15} (PAPER.md §5.3.1; `examples/sigma_floor_sweep.*`) shows the matched-cell-line AUC ranking is robust (≤0.02 movement across a 7.5×-range sweep) but tissue AUC peaks at σ_floor = 0.10, not the shipped default. Formal regime-specific σ_floor default selection is deferred.
...
4. **Optional R `limma` parity check.** Our limma-eBayes baseline is a pure-python Smyth (2004) implementation (§4.3, §5.2.2); a supplementary R-vs-python parity table on a small matrix would close the remaining methods-journal concern about faithfulness to the canonical `limma::lmFit` + `limma::eBayes` call path.
...
From a fresh clone of the repository at `memo-2026-04-22-ao`:
