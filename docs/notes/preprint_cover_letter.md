...
**Manuscript reference.** `PAPER.md` (or `MANUSCRIPT.md` for the Bioinformatics-shaped version) in <https://github.com/AllisonH12/thermocas9> at tag `memo-2026-04-22-ag`.
...
We are submitting our manuscript, *"Compositional probabilistic scoring and tie-band-aware benchmarking for methylome-guided ThermoCas9 target-site ranking"*, for consideration.
...
The paper's core contribution is a **compositional probabilistic scoring skeleton** paired with a **tie-band-aware benchmarking contract** specialized to the methylation-sensitive-Cas9 ranking problem. The skeleton is
`p_therapeutic_selectivity = p_targetable_tumor × (gap factor) × p_observation_trustworthy`,
where each factor is independently replaceable — the specific choice of gap factor is an instance, not a load-bearing methodological claim. The shipped instance (V2.5) uses `p_differential_protection = P(β_normal − β_tumor > δ)` under an independent-normal approximation on per-probe β summaries, with a σ floor handling low-replicate IQRs. A factor-ablation experiment in §5.2.1 clarifies what the specific `p_diff` shape does and does not do: on matched cell-line cohorts it matches a fixed-bandwidth sigmoid within 0.001 AUC (the σ-floor is the binding constraint on ~99% of records, so the per-site σ adaptation is immaterial there); on primary tissue, a bandwidth-robust fixed-bandwidth sigmoid ablation actually *beats* shipped `p_diff` by +0.05 to +0.09 AUC across four tested bandwidths. This pushes the load-bearing contribution to the compositional skeleton plus the benchmark contract — not to the specific gap factor shipped today — and it names a concrete next-version tissue mode (`gap_sigmoid`) as the immediate follow-up rather than a diffuse "future work". The scorer is benchmarked on four public methylation cohorts (GSE322563 matched cell lines as the independent primary endpoint, GSE77348 as the development cohort on which the differential threshold δ was tuned, GSE69914 as 305 sporadic breast tumors + 50 healthy-donor breast tissue samples — unpaired by design with adjacent-normal arms excluded by the cohort build script, GSE68379 as the out-of-distribution boundary) against a 3-tier positives set derived from the Roth paper's validated targets. We also ship:
...
Benchmark `BenchmarkResult` JSONLs, positives lists, annotated top-20 TSV + Markdown shortlists, figures, and the test suite are all committed at the immutable tag `memo-2026-04-22-ag`. The large per-cohort scored-candidate JSONLs (`data/derived/scored_*.jsonl`, tens of millions of records each) are gitignored but fully reproducible from the committed build scripts and cohort YAMLs — the exact `uv run` invocations are listed in the manuscript's reproducibility appendix. 243 unit tests pass.
...
- **Tag-reference check before submission**: `git rev-parse memo-2026-04-22-ag` must still resolve, and both `PAPER.md` and `MANUSCRIPT.md` citations must still read `memo-2026-04-22-ag`. Do not let development on `main` implicitly move the reference.
