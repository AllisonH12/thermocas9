...
**Date.** 2026-04-22.
**Code.** <https://github.com/AllisonH12/thermocas9> at tag `memo-2026-04-22-l` (immutable pointer to the exact revision that produced this memo; supersedes `memo-2026-04-22-k` with three documentation-drift fixes: README.md `tumor_only` tie-band claim corrected from "6,500+ on every cohort" to the verified 5,271-14,914 enumeration plus full 4-cohort-path enumeration in the V2.5 mode row; `verify_manuscript_claims.py` extended to scan README.md (was previously scoped to MANUSCRIPT.md and PAPER.md only); CHANGELOG.md caught up to document the entire `-c` through `-k` submission-freeze cycle; toolchain-version reproducibility caveat surfaced in this ledger entry and in README.md, not just in the render-script header).
...
- **Code**: <https://github.com/AllisonH12/thermocas9>. Five dated memo tags plus the stable-release tag, per the immutable-tag policy (never move; supersede by cutting a new dated tag):
  ...
  - `memo-2026-04-22-l` — **the exact revision that produced this memo.** README.md `tumor_only` tie-band line corrected to enumerate "5,271–14,914 across the five cohort paths tested" with per-cohort breakdown; V2.5 mode-table row updated to enumerate all four cell-line-cohort paths and the corrected "highest-AUC discovery axis at every non-boundary cohort × tier" framing. `verify_manuscript_claims.py` extended to scan README.md and to detect the "6,500+ on every cohort" pattern (regression-tested by injection). CHANGELOG.md caught up with an Unreleased block covering the Δβ-only baseline, MANUSCRIPT.md, render-pipeline reproducibility, figure regeneration, the verify script, and the dated memo-tag policy. Toolchain-version caveat surfaced in PAPER.md (this entry), README.md (new "Reproducibility" section), and the render-script header (where it already lived). Resolve to a SHA with `git rev-parse memo-2026-04-22-l` in a fresh clone.
  Development continues on `main` past the tagged memo revision; cite `memo-2026-04-22-l` when citing this document.
