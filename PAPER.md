...
**Date.** 2026-04-22.
**Code.** <https://github.com/AllisonH12/thermocas9> at tag `memo-2026-04-22-m` (immutable pointer to the exact revision that produced this memo; supersedes `memo-2026-04-22-l` with two fixes: cover-letter `docs/notes/preprint_cover_letter.md` mislabel of GSE69914 as "tumor-normal pairs" corrected to the actual unpaired sporadic-tumor + healthy-donor composition (matching MANUSCRIPT.md §4.1); and `scripts/verify_manuscript_claims.py` docstring narrowed to honestly match the implementation — the prior docstring claimed "every AUC value in every markdown table" but the corresponding `check_table_auc_values` is unwired, and "all three documents" overstated README coverage. The narrowed docstring enumerates exactly which `check_*` functions are wired into `main()`, what each one verifies, and lists three known coverage gaps explicitly).
...
- **Code**: <https://github.com/AllisonH12/thermocas9>. Five dated memo tags plus the stable-release tag, per the immutable-tag policy (never move; supersede by cutting a new dated tag):
  ...
  - `memo-2026-04-22-m` — **the exact revision that produced this memo.** Cover-letter cohort wording corrected to match MANUSCRIPT.md §4.1. Verify-script docstring rewritten to enumerate exactly the `check_*` functions wired in `main()` (constants on §2.1; ordering invariants on the JSONL grid; tumor_only tie-band per-cohort + stale-pattern sweep across all three docs; artifact count; test count; figure-caption gene-list/2-column-framing checks on MANUSCRIPT.md and PAPER.md), and to explicitly list three known coverage gaps: per-cell AUC verification (function exists but unwired due to noisy heuristic), README scope narrower than the manuscripts, and `docs/notes/` not scanned. Resolve to a SHA with `git rev-parse memo-2026-04-22-m` in a fresh clone.
  Development continues on `main` past the tagged memo revision; cite `memo-2026-04-22-m` when citing this document.
