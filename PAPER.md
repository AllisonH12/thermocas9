...
**Date.** 2026-04-22.
**Code.** <https://github.com/AllisonH12/thermocas9> at tag `memo-2026-04-22-f` (immutable pointer to the exact revision that produced this memo; supersedes `memo-2026-04-22-e` which still carried a false-universal `at every cohort × tier combination tested` claim that is violated on 3 OOD rows, a false ordering claim `V2.5 > V1 > Δβ` violated on 7 of 9 matched-cell-line rows, and a wrong normal-arm mechanism attribution in §5.3/§6.1 — all corrected here. A new `scripts/verify_manuscript_claims.py` guard enforces that every numeric / universal claim in PAPER.md and MANUSCRIPT.md matches the committed artifacts and source constants; this should have existed before `-c`).
...
- **Code**: <https://github.com/AllisonH12/thermocas9>. Five dated memo tags plus the stable-release tag, per the immutable-tag policy (never move; supersede by cutting a new dated tag):
  ...
  - `memo-2026-04-22-f` — **the exact revision that produced this memo.** Corrects all three of the above. Also adds `scripts/verify_manuscript_claims.py`: a programmatic guard that cross-checks every numeric / universal claim in PAPER.md and MANUSCRIPT.md against the committed bench JSONLs and `src/thermocas/probabilistic.py` constants, exiting non-zero on mismatch. This script must pass before any subsequent dated memo tag is cut. Resolve to a SHA with `git rev-parse memo-2026-04-22-f` in a fresh clone.
  Development continues on `main` past the tagged memo revision; cite `memo-2026-04-22-f` when citing this document.
