...
**Date.** 2026-04-22.
**Code.** <https://github.com/AllisonH12/thermocas9> at tag `memo-2026-04-22-g` (immutable pointer to the exact revision that produced this memo; supersedes `memo-2026-04-22-f` which lacked Figures 2 and 3 in MANUSCRIPT.md — fig2 (cross-cohort AUC bars) and fig3 (top-20 gene heatmap) have now been regenerated against the 4-axis × 4-cohort-path committed bench grid (including Δβ-only naive bars and the GSE322563 native EPIC v2 cohort) and embedded in MANUSCRIPT.md §5).
...
- **Code**: <https://github.com/AllisonH12/thermocas9>. Five dated memo tags plus the stable-release tag, per the immutable-tag policy (never move; supersede by cutting a new dated tag):
  ...
  - `memo-2026-04-22-g` — **the exact revision that produced this memo.** Regenerates `docs/figures/fig2_auc_bars.{png,svg}` against the full 4-axis × 4-cohort-path × 3-tier committed bench grid; regenerates `docs/figures/fig3_topgene_heatmap.{png,svg}` to include the GSE322563 native EPIC v2 column (5 columns total). Embeds both figures in MANUSCRIPT.md §5 with captions verified against committed data (caption claim "GSE69914 tissue shares zero genes with cell-line V2.5 shortlists" is a real cell-line-vs-tissue biological-shift finding, not a claim I had verified before this round). Resolve to a SHA with `git rev-parse memo-2026-04-22-g` in a fresh clone.
  Development continues on `main` past the tagged memo revision; cite `memo-2026-04-22-g` when citing this document.
