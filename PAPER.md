...
**Date.** 2026-04-22.
**Code.** <https://github.com/AllisonH12/thermocas9> at tag `memo-2026-04-22-p` (immutable pointer to the exact revision that produced this memo; see the tag ledger in §"Data and code availability" for the per-tag history of the submission-freeze cycle).
...
- **Code**: <https://github.com/AllisonH12/thermocas9>. Stable-release tag plus the per-revision dated memo tags from the submission-freeze cycle, per the immutable-tag policy (never move; supersede by cutting a new dated tag). The authoritative list is `git tag -l "memo-*"` at this revision; enumerated here with per-tag provenance:
  ...
  - `memo-2026-04-22-j` — fixed five issues (PDF body metadata leak, non-reproducible date, three PAPER.md drift items). **Retained but SHOULD NOT BE CITED**: PDF embedded `CreationDate` / `ModDate` were still wall-clock at render time, so two renders of the same tagged source produced PDFs that diff at the metadata level. Page content was identical, so this is a low-priority artifact-reproducibility issue rather than a content blocker.
  - `memo-2026-04-22-k` — added `SOURCE_DATE_EPOCH` export to render pipeline (deterministic embedded `CreationDate` / `ModDate`); both PDFs verified byte-identical across consecutive re-renders. **Reproducibility caveat (also documented in `scripts/render_paper_pdf.sh` header)**: byte-identity holds within the same toolchain version pair (verified: `pandoc 3.9` + `typst 0.14.2`). A different `typst` minor version will produce a different PDF from the same source even with `SOURCE_DATE_EPOCH` set, because the renderer itself is non-stable across versions; that is renderer drift, not pipeline drift. **Retained but SHOULD NOT BE CITED**: README.md still had the pre-correction `tumor_only` tie-band claim "6,500+ on every cohort tested" (actual range across 5 cohort paths is 5,271-14,914) and only enumerated 2 of the 4 cell-line-cohort paths in the V2.5 mode row. CHANGELOG.md was also significantly behind: the entire `-c` through `-k` submission-freeze cycle was undocumented in the changelog.
  - `memo-2026-04-22-l` — README.md tumor_only tie-band drift fixed; verifier extended to scan README; CHANGELOG.md caught up; toolchain caveat surfaced beyond the script header. **Retained but SHOULD NOT BE CITED**: cover letter still misdescribed GSE69914 as "tumor-normal pairs" (the cohort is unpaired by design — 305 sporadic tumors + 50 healthy donors with adjacent-normal arms excluded); verify-script docstring overstated coverage by claiming "every AUC value in every markdown table" when `check_table_auc_values` is in fact unwired, and by implying full README coverage when only the tumor_only tie-band pattern is checked there.
  - `memo-2026-04-22-m` — corrected cover-letter cohort wording and made the verifier docstring honest about its scope. **Retained but SHOULD NOT BE CITED**: README.md "Reproducibility" section (added in `-l`) overclaimed by saying "any `memo-2026-04-22-*` tag" produces byte-identical PDFs, when in fact `-i` and earlier had a title-block-strip bug that leaked metadata into the body and `-j` still embedded wall-clock PDF timestamps. CHANGELOG.md also still cited `memo-2026-04-22-l` as the submission anchor when MANUSCRIPT/PAPER/notes had moved to `-m`.
  - `memo-2026-04-22-n` — README.md reproducibility scope narrowed to `-k` and later; CHANGELOG.md submission anchor bumped to `-n`. **Retained but SHOULD NOT BE CITED**: `**Code.**` line 5 of PAPER.md opened a parenthetical after the tag citation but never closed it before the line ended (the inner nested parenthetical closed correctly, the outer did not). Visible in the rendered PDF as unfinished provenance text in the first metadata block.
  - `memo-2026-04-22-o` — fixed the PAPER.md `**Code.**`-line unclosed parenthetical by collapsing to a one-sentence tag citation pointing to this ledger. **Retained but SHOULD NOT BE CITED**: five stale count/span claims in narrative prose that drifted as more tags were cut — PAPER.md tag-ledger header said "Five dated memo tags" (16 actual), MANUSCRIPT.md said "Supersedes prior same-day tags `-c`, `-d`, `-e`" (actually supersedes through -n), and CHANGELOG.md had three stale spans (`-c` through `-n`, `-c` → `-h`, `-c` through `-l`) in narrative prose. Same class of count/enumeration drift that has recurred across the freeze cycle.
  - `memo-2026-04-22-p` — **the exact revision that produced this memo.** Rewrote all five stale count/span claims to be count-free where possible ("Stable-release tag plus the per-revision dated memo tags ...", "all prior same-day dated memo tags", "the same-day revision cycle from `-c` onward", "the `-c` → `-e` re-cuts" for the fabricated-numbers class specifically, "the `-c` onward cycle"). Extended `scripts/verify_manuscript_claims.py` with a `check_tag_span_claims` detector that catches the class going forward: flags "-c through -X" (and "`-c` → `-X`") where X isn't the latest memo-2026-04-22-* suffix on disk (with a historical-context filter exempting tag-ledger retrospective entries), and flags "N dated memo tags" where N doesn't match `git tag -l`. Regression-tested: detector catches an injected "-c through -j" and an injected "Five dated memo tags" claim; detector stays silent on the intentional historical retrospectives in the ledger. Resolve to a SHA with `git rev-parse memo-2026-04-22-p` in a fresh clone.
  Development continues on `main` past the tagged memo revision; cite `memo-2026-04-22-p` when citing this document.
