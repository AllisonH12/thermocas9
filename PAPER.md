...
**Date.** 2026-04-22.
**Code.** <https://github.com/AllisonH12/thermocas9> at tag `memo-2026-04-22-n` (immutable pointer to the exact revision that produced this memo; supersedes `memo-2026-04-22-m` with two scope-correction fixes: README.md's "Reproducibility" section narrowed from "any `memo-2026-04-22-*` tag" (overclaim — `-i` had a title-block-strip bug that leaked metadata into the rendered PDF body, `-j` still embedded wall-clock `CreationDate`/`ModDate`) to "`memo-2026-04-22-k` and later" with explicit per-tag history; and CHANGELOG.md submission-anchor citation bumped from `-l` to `-n` with a brief tag-delta block describing what `-m` and `-n` corrected.
...
- **Code**: <https://github.com/AllisonH12/thermocas9>. Five dated memo tags plus the stable-release tag, per the immutable-tag policy (never move; supersede by cutting a new dated tag):
  ...
  - `memo-2026-04-22-n` — **the exact revision that produced this memo.** README.md "Reproducibility" section narrowed from "any `memo-2026-04-22-*` tag" to "`memo-2026-04-22-k` and later", with an explicit per-tag history note explaining what `-i` and `-j` lacked. CHANGELOG.md submission anchor bumped `-l` → `-n` and an inline tag-delta block describes what `-m` and `-n` corrected. Resolve to a SHA with `git rev-parse memo-2026-04-22-n` in a fresh clone.
  Development continues on `main` past the tagged memo revision; cite `memo-2026-04-22-n` when citing this document.
