...
**Date.** 2026-04-22.
**Code.** <https://github.com/AllisonH12/thermocas9> at tag `memo-2026-04-22-k` (immutable pointer to the exact revision that produced this memo; supersedes `memo-2026-04-22-j` by adding `SOURCE_DATE_EPOCH` export to the render pipeline, derived from the source `**Date.**` line — Typst now embeds a deterministic CreationDate/ModDate so two renders of the same tagged source produce byte-identical PDFs (verified by md5 across consecutive runs with a wall-clock gap)).
...
- **Code**: <https://github.com/AllisonH12/thermocas9>. Five dated memo tags plus the stable-release tag, per the immutable-tag policy (never move; supersede by cutting a new dated tag):
  ...
  - `memo-2026-04-22-k` — **the exact revision that produced this memo.** Render script now exports `SOURCE_DATE_EPOCH` from the source `**Date.**` line (interpreted at 00:00:00 UTC) before invoking `pandoc --pdf-engine=typst`; Typst respects this and embeds deterministic `CreationDate` / `ModDate`. Verified: two consecutive renders of the same source produce byte-identical PDFs (same md5). Both `MANUSCRIPT.pdf` and `PAPER.pdf` re-rendered against the fixed pipeline; both committed. Resolve to a SHA with `git rev-parse memo-2026-04-22-k` in a fresh clone.
  Development continues on `main` past the tagged memo revision; cite `memo-2026-04-22-k` when citing this document.
